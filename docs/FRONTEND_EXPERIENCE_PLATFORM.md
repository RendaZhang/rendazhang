...
- `docs/THEME_PALETTE_TOKEN_MODEL.md` 已定义 theme mode、palette 和 accent 的后续 token/DOM/storage 边界。
...
| `palette` | 一组基础品牌色、强调色、渐变和可读性约束 | 未来 token 设计切片决定 | 不直接等同于 light/dark；应有独立 DOM 表达和 fallback |
...
`src/stores` 只能保存小型客户端 UI/preference 状态。后续 palette/accent 状态如果加入，应满足：
...
`ThemeProvider` 继续拥有现有主题 mode 的解析、context、DOM `data-theme` 同步和存储写入。后续如果添加 palette provider 或扩展现有 provider，必须说明：

- SSR 初始状态和客户端恢复顺序。
- 与 `/js/base-layout-init.js` 的关系。
- 是否需要新的 DOM attribute，例如 `data-palette`。
...
- palette 数量少，影响范围窄。
- 使用现有导航/主题控制视觉语言，不做大范围 redesign。
- 验证移动端布局、按钮文本/图标不溢出、对比度、storage、hydration 和 browser smoke。
