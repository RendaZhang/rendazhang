...
Phase 8 现已完成。后续围绕首屏观感、palette 感知、交互反馈、Chat Widget 外壳和移动端视觉
QA 的工作，统一进入 [外观与交互 Polish](./VISUAL_INTERACTION_POLISH.md)。
...
- Phase 8 的平台基础已关闭为 `Done`；Phase 9 不重新打开这些平台 slice，而是在该基础上做视觉与
  交互 polish。
...
- modal、popover/menu、toast/status、loading、键盘行为和 focus 管理见：[交互组件标准](./INTERACTION_COMPONENT_STANDARDS.md)。
- 首屏观感、palette 感知、交互反馈、Chat Widget 外壳和移动端视觉 QA 见：
  [外观与交互 Polish](./VISUAL_INTERACTION_POLISH.md)。
