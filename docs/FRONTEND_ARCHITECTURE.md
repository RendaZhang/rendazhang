...
后续体验平台规划见：[前端体验平台 RFC](./FRONTEND_EXPERIENCE_PLATFORM.md)。该 RFC 记录浏览器 smoke、hydration 安全、主题 mode 与 palette/accent 分离、调色板切片顺序和交互标准的后续边界。theme mode、palette 和 accent 的具体 token/DOM/storage 模型见：[主题调色板 Token 模型](./THEME_PALETTE_TOKEN_MODEL.md)。
