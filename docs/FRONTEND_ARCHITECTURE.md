...
后续体验平台规划见：[前端体验平台 RFC](./FRONTEND_EXPERIENCE_PLATFORM.md)。该 RFC 记录浏览器 smoke、hydration 安全、主题 mode 与 palette/accent 分离、调色板切片顺序和交互标准的后续边界。theme mode、palette 和 accent 的具体 token/DOM/storage 模型见：[主题调色板 Token 模型](./THEME_PALETTE_TOKEN_MODEL.md)。modal、popover/menu、toast/status、loading、键盘行为、focus 管理和 browser smoke 门禁见：[交互组件标准](./INTERACTION_COMPONENT_STANDARDS.md)。
...
- 主题 mode、palette、语言、页面标题和登录状态的渲染前初始化。
...
- 当前主题 mode。
- `default`、`aurora` 和 `forest` 调色板偏好边界。
- Chat Widget 打开/关闭状态。
- 偏好状态是否已经由客户端 provider 同步完成。

该 store 是跨 island 读取边界，不替代现有 provider：

- `ThemeProvider` 仍拥有主题 context、DOM `data-theme` 同步和 `preferred_theme` 写入，同时把已解析主题 mode 发布到 store。
- `ThemeProvider` 同步 `data-palette` 和 `preferred_palette`，但不把 palette 名称写入 `data-theme`。
- `ThemeToggle` 等主题控制组件读取 store snapshot 判断当前选中状态，主题 mode 和 palette 写入仍通过 `ThemeProvider` 统一执行。
...
- HTTP 错误对象。
- 未授权跳转或跳过未授权跳转的显式选项。
- 共享的 line streaming transport，用于需要复用 fetch、认证和 abort 行为的专用服务。
...
`src/services/chatService.ts` 仍是 Chat 流式响应的专用领域服务。它通过 `apiClient` 复用 `fetch` 默认值、`credentials: include`、401 跳转、HTTP 错误和可选 `AbortSignal` 处理，但继续拥有 `/deepseek_chat/` 返回行的 Chat 语义解析、assistant 文本累积、流式 chunk 回调和 reset 的布尔返回约定。普通 JSON API 不应复用 Chat 服务路径。

`src/services/contactService.ts` 拥有当前联系表单的第三方 Formspree 提交路径。它保持旧有 URL-encoded payload、Formspree endpoint 和 `{ ok: true }` 成功判定，不走 CloudChat `apiClient`，也不应作为新站内 JSON API 的模板。
...
- palette/accent 不应复用 `data-theme` 表达；当前 palette 使用 `data-palette` 和
  `preferred_palette`，具体设计见：[主题调色板 Token 模型](./THEME_PALETTE_TOKEN_MODEL.md)。
...
- 新增主题调色板、复杂交互或 hydration-sensitive UI 前，先参考 [前端体验平台 RFC](./FRONTEND_EXPERIENCE_PLATFORM.md) 中的 Phase 8 切片顺序和验证门禁。
- 新增 modal、popover/menu、toast/status、loading、empty/error state、键盘行为或 focus 管理前，先参考 [交互组件标准](./INTERACTION_COMPONENT_STANDARDS.md)。
