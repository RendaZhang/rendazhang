...
当前 provider 状态集中在 `src/components/NavBarWrapper.tsx` 内：
...
这些 provider 当前服务于导航及其包裹的 children。跨 island 的共享状态尚未作为通用能力解决。不要为了单个页面临时创建全局 store 或复制 provider 状态；全局客户端 store 应等待后续 Slice 3.1 统一设计。
...
`src/services/chatService.ts` 当前仍是 Chat 流式响应的专用路径。它负责 `/deepseek_chat/` 相关的流式 NDJSON 读取、取消控制、重置会话和 Chat 专属错误处理。该路径在后续 Chat controller 或 API client 重构切片前保持独立，不应被普通 JSON API 调用复用。
...
- 设计 token 属于 `src/styles/core/tokens.css` 和相关 core 文件。
