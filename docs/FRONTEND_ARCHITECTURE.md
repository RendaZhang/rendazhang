...
- 当前主题 mode。
- 未来 palette 控制使用的 `default` 调色板偏好边界。
- Chat Widget 打开/关闭状态。
...
- palette 偏好当前只提供 store 默认值、枚举校验和 `preferred_palette` 持久化 helper；还没有用户可见控制、
  `data-palette` DOM 同步或 palette CSS token 覆盖。
...
- 后续 palette/accent 不应复用 `data-theme` 表达；具体设计见：[主题调色板 Token 模型](./THEME_PALETTE_TOKEN_MODEL.md)。
