...
- `ChatWidget` 拥有同源 iframe、ready skeleton、outside click close、
  `aria-expanded` / `aria-controls` launcher state 和 `chat-enhancement-ready` 接收逻辑。
...
- Chat Widget 外层 loading skeleton 只在 iframe wrapper 内做视觉结构化，占位层保持
  `aria-hidden="true"`；父级可读忙碌状态继续由 wrapper 的 `aria-busy` 表达。
