...
本切片不改变运行时 CSS、TypeScript、UI、依赖、后端、Nginx 或生产服务器状态。

## 当前事实

- 当前只有 light/dark `theme mode`，由 `BaseLayout` 预绘制脚本、`ThemeProvider`、
  `html[data-theme]`、`preferred_theme` storage 和 `uiPreferencesStore` 共同维护。
- `ThemeProvider` 仍拥有 DOM `data-theme` 同步和主题存储写入；store 只是跨 island
  读取边界。
...
- `src/styles/core/theme-tokens.css` 放当前主题语义别名，以及
  `html[data-theme='dark']` 覆盖。
- `src/styles/core/_gradients.css` 放基于基础色和语义覆盖的渐变 token。
- `npm run smoke:browser` 已覆盖首页、`/deepseek_chat/`、Chat Widget iframe ready 和当前主题
  mode 切换的 DOM/storage/console 行为。
...
| `palette` | 尚未实现。当前品牌紫、蓝、渐变和语义色可视为未来 `default` palette 的基线。 | 表达一组品牌色、强调色、语义映射和渐变约束；应独立于 light/dark。 |
...
- 用户可见控制必须等 token 模型、store readiness 和 browser smoke gate 都明确后再做。

## DOM、存储与 readiness 模型

### DOM attribute

后续 palette 的推荐 DOM 表达是 `html[data-palette]`。默认 palette 建议命名为
`default`，但本切片不新增该 attribute。原因是当前 CSS 已以 `:root` 定义现有配色，
在没有 palette UI 前添加 DOM 状态只会增加 hydration 面。
...
html[data-palette='aurora'] {
  /* future palette raw/semantic overrides */
}
...
`data-accent` 暂不作为 Slice 8.3 的默认要求。只有当 palette 内需要多个局部强调色，且
...
未来 palette 原始值可以放在这里，但应保持清晰分组，例如：

```css
/* Future example only; not implemented in this slice. */
...
未来 palette 应优先在这一层把原始 palette 值映射到语义变量，例如：
...
未来 palette 改变渐变时，应优先让渐变引用语义或 palette 原始 token，而不是在按钮、
...
未来第一个用户可见 palette MVP 应优先控制影响面。预计最小需要关注：
...
- 如果 provider 要同步 `data-palette`，应记录是否扩展现有 `ThemeProvider`，或新增更小的
  palette provider。
- 如需预绘制写入 `data-palette`，必须走外部 `base-layout-init` 路径，并评审 CSP。
- Slice 8.3 默认不新增用户可见 UI；Slice 8.4 才实现控制 MVP。
...
- 新 theme switcher UI。
- 新 palette/accent store 状态实现。
- 运行时 CSS token 改名或视觉变化。
