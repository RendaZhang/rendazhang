...
- [样式说明](#%E6%A0%B7%E5%BC%8F%E8%AF%B4%E6%98%8E)
  ...
  - [排版与间距](#%E6%8E%92%E7%89%88%E4%B8%8E%E9%97%B4%E8%B7%9D)
    - [示例](#%E7%A4%BA%E4%BE%8B)
      - [✅ Do](#-do)
      - [❌ Don't](#-dont)
      - [容器查询注意事项](#%E5%AE%B9%E5%99%A8%E6%9F%A5%E8%AF%A2%E6%B3%A8%E6%84%8F%E4%BA%8B%E9%A1%B9)
...
- Palette 的视觉影响应优先通过 `html[data-palette]` 和语义 token 扩展，禁止在组件里散落硬编码
  品牌色。
- 首页、导航、社交入口和 Chat Widget 等高感知 surface 应优先读取
  `--color-palette-surface`、`--color-palette-surface-strong`、`--color-palette-border`、
  `--color-hero-accent` 和 `--color-hero-scrim*` 这类语义 token，而不是直接读取
  `--palette-*` 原始值。
...
- 根据布局需要选用 `--bp-sm`、`--bp-sm-2` 等断点 token 和 `--container-sm` 等容器宽度 token，但在 `@container`/`@media` 查询条件中必须写死具体长度值，可在构建期通过 PostCSS mixins 展开以避免 `var()` 被忽略。

#### ❌ Don't

- 直接使用 `px` 指定字号或间距。
- 在同一页面混用不同单位导致排版不一致。
- 在 `@container` 或 `@media` 条件中使用 `var(--bp-*)`。

#### 容器查询注意事项

- `@container` 与 `@media` 的查询条件必须使用可解析的常量，禁止在条件中使用 `var(.*)` 等自定义属性，否则浏览器会忽略整段规则（详见 BUG-057）。
- 若需要复用断点，可在构建期通过 PostCSS Mixins 或 Sass 将别名展开为 `@container (max-width: 48rem)` 等标准语法；组件根元素应显式声明 `container-type`。

```css
.navigation { container-type: inline-size; }

@container (max-width: 48rem) {
  .navigation .menu { flex-direction: column; }
}
```
...
- `@container`/`@media` 查询条件必须在解析期可确定，禁止直接使用 `var()` 或 `@custom-media` 作为断点；如需别名，请在构建期通过 PostCSS mixins 展开为明确长度值。
