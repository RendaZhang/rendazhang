...
- [项目需求清单](#%E9%A1%B9%E7%9B%AE%E9%9C%80%E6%B1%82%E6%B8%85%E5%8D%95)
  ...
  - [🚀 核心功能体系](#-%E6%A0%B8%E5%BF%83%E5%8A%9F%E8%83%BD%E4%BD%93%E7%B3%BB)
    ...
    - [用户账户体系](#%E7%94%A8%E6%88%B7%E8%B4%A6%E6%88%B7%E4%BD%93%E7%B3%BB)
...
- **认证页过渡动画**：登录、注册等表单提交成功后展示全屏加载覆盖层，缓解跳转等待
...
### 用户账户体系
- **登录表单**：邮箱或用户名 + 密码、记住我、密码强度提示
- **已登录用户跳转**：若用户已登录，直接访问 `/login/` 时自动重定向至首页，避免重复登录。
- **注册流程**：邮箱、用户名、密码确认、进度条
- **第三方登录**：Google、微信登录/注册
- **忘记/重置密码**：
  - 忘记密码页提交邮箱至 `POST /cloudchat/auth/password/forgot`，无论邮箱是否存在均提示“如果该邮箱存在，我们已发送重置邮件，请在 15 分钟内完成重置。”
  - 重置页 `/reset_password#token=...` 校验 token，兼容旧查询参数链接并在读取后立即清理地址栏；输入新密码并确认（8–128 位，至少两类字符），调用 `POST /cloudchat/auth/password/reset`。
  - 重置成功提示“密码已重置，请使用新密码登录。”并强制旧会话下线。
- **状态管理**：`AuthProvider` 先从本地标记初始化；只有存在本地登录信号时才调用 `/auth/me` 校验并保持 DOM 属性同步。公共未登录页面没有本地信号时不主动探测该端点；登出、401 校验失败及密码重置后会清理本地标记，避免 401 重定向循环和公共页 console 噪音
- **用户账号信息页**：提供查看和管理用户账号资料的页面，从汉堡菜单入口访问
...
- [x] **页面风格统一**
  ...
  - 配色体系重构：保持视觉一致性又增强界面层次感，所有颜色与渐变集中在 `src/styles/core/tokens.css` 与 `_gradients.css`，变量附带用途注释并在 `tokens.md`、`_gradients.md` 文档记录，组件禁止硬编码颜色。
- [x] **页面初级重构**
  ...
  - 导航栏与汉堡菜单的交互依赖 DOM，现改为 `client:load` 先渲染静态结构再在客户端水合，避免 SSR Hydration 错误
...
- [x] **登录与注册基础功能**
  - 支持邮箱或用户名登录，记住我与密码强度提示
  - 注册表单包含邮箱、用户名、密码确认及进度条
  - 提供 Google、微信第三方入口（按钮暂未启用）
  - 登录成功后，汉堡菜单展示账号信息入口并可跳转至 Account Profile 页面
- [x] **性能增强方案**
  - 所有图片资源懒加载
...
- [x] **TypeScript 迁移**
  ...
  - 阶段4：全量类型检查
- [x] **API 客户端请求策略**
  - `apiClient` 统一封装 `fetch` 调用
  - 所有请求强制携带 `credentials: 'include'`，外部无法覆盖
  - `GET` 请求自动移除 `Content-Type` 头，其他方法默认合并 `JSON_HEADERS`
  - 异常时对请求参数进行脱敏后上报 Sentry
