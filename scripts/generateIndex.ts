...
const BARREL_EXCLUDED_MODULES = new Set([
  // Keep lazy-only modules out of generated barrels so Vite/Rolldown can preserve dynamic chunks.
...
function isBarrelExcluded(absPath: string): boolean {
  return BARREL_EXCLUDED_MODULES.has(pathForLog(absPath));
}
...
async function generate(dir: string): Promise<GenerateResult> {
  ...
  dirs.sort();
  tsFiles.sort();
  const exportableTsFiles = tsFiles.filter((f) => !isBarrelExcluded(path.join(dir, f)));
  ...
  const hasTs = hasIndexTs || exportableTsFiles.length > 0 || subResults.some(([, r]) => r.hasTs);
  ...
  // 当前目录文件
  for (const f of exportableTsFiles) {
